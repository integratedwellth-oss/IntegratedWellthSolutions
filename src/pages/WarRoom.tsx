...
import ComplianceCalendar from '../components/ComplianceCalendar'; // Path corrected for pages folder
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid lg:grid-cols-12 gap-6">
          <main className="lg:col-span-8 space-y-6">
            <section className="bg-[#0a0c12] border border-white/5 rounded-2xl p-8 shadow-2xl">
              ...
              <div className="grid md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Cash Reserves (R)</label>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-[0.2em]">Revenue Shock ({shock}%)</label>
              ...
              <div className="mt-12 grid grid-cols-2 gap-6 pt-8 border-t border-white/5">
                <div className="p-4 bg-white/[0.02] rounded-xl border border-white/5">
                  <span className="block text-[10px] text-gray-500 uppercase mb-2 font-black">Survival Runway</span>
                  <span className="text-3xl font-black text-white">
                    {calculations.runway === Infinity ? 'SOVEREIGN' : `${Math.floor(calculations.runway)} Mo`}
                  </span>
                </div>
                <div className="p-4 bg-white/[0.02] rounded-xl border border-white/5">
                  <span className="block text-[10px] text-gray-500 uppercase mb-2 font-black">Adjusted Net Flow</span>
                  <span className={`text-3xl font-black ${calculations.netFlow >= 0 ? 'text-emerald-400' : 'text-red-500'}`}>
          ...
          <aside className="lg:col-span-4 space-y-6">
            <div className="bg-[#0a0c12] border border-brand-gold/20 rounded-2xl p-6">
              ...
              <div className="p-4 bg-white/5 rounded-xl border-l-2 border-brand-gold mb-6">
                <p className="text-xs text-white leading-relaxed font-bold italic">"{calculations.status.tip}"</p>
              </div>
              <ul className="space-y-3 mb-8">
                <li className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-tighter"><ShieldCheck size={12} className="text-brand-gold"/> Audit Behavioral Leaks</li>
                <li className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-tighter"><ShieldCheck size={12} className="text-brand-gold"/> Tax Governance Sync</li>
              </ul>
              <button className="w-full py-4 bg-brand-gold text-brand-900 font-black uppercase text-[10px] tracking-widest rounded-xl hover:bg-white transition-all flex items-center justify-center gap-2">
                Initiate Strategic Triage <ArrowRight size={14} />
              </button>
            </div>
          </aside>
        </div>
