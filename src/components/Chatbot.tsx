...
import { functions } from '../firebaseConfig'; // Adjust import path
...
export const Chatbot: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'bot', text: 'Hello! I am the website assistant. How can I help you today?' }
  ]);

  // Load history on mount
  useEffect(() => {
    const saved = localStorage.getItem('website_chat_history');
    if (saved) setMessages(JSON.parse(saved));
  }, []);

  // Save history and auto-scroll on new message
  useEffect(() => {
    localStorage.setItem('website_chat_history', JSON.stringify(messages));
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Ensure the name here matches your exported function name
