  ...
  const handleAiAppraisal = async () => {
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        if (text) setAiAnalysis(prev => prev + text);
      }
    } catch (e) {
          ...
          <div className="lg:col-span-5 space-y-12">
            <RevealOnScroll>
              ...
              <p className="text-2xl text-white font-medium leading-relaxed mt-10 border-l-4 border-brand-gold/40 pl-10">
                Compliance is binary. You are either <span className="text-emerald-400 font-bold">Safe</span> or you are <span className="text-rose-500 font-bold">Exposed</span>.
              </p>
            </RevealOnScroll>
          </div>
                ...
                <div className="flex-1">
                   {isSuccess ? (
                    <div className="space-y-12 animate-fadeIn py-10 text-center">
                      ...
                      <button onClick={handleDownloadIntel} className="w-full flex items-center justify-center gap-4 py-8 rounded-[2.5rem] bg-brand-gold text-brand-900 font-black uppercase tracking-[0.4em] text-sm transition-all hover:scale-105 shadow-2xl"><FileText size={18} /> DOWNLOAD RECOVERY PDF</button>
                    </div>
                  ) : isTransmitting ? (
                    <div className="flex flex-col items-center justify-center h-full space-y-10 py-20 animate-fadeIn">
                      ...
                      <div className="space-y-10 animate-fadeIn">
                         <div className="flex justify-between items-center bg-black/60 p-8 rounded-[2rem] border border-white/10">
                          ...
                          <button onClick={handleAiAppraisal} disabled={isAnalyzing} className="w-full flex items-center justify-center gap-4 bg-white/10 border border-white/20 text-white rounded-[2rem] py-6 font-black uppercase tracking-[0.2em] text-[10px] transition-all hover:bg-white/20">
                            {isAnalyzing ? <Loader2 className="animate-spin" size={16} /> : <Cpu size={16} />} ANALYZE RISK
                          </button>
                          {aiAnalysis && <div className="p-8 bg-brand-gold/10 border-l-4 border-brand-gold text-white/90 rounded-r-2xl text-sm leading-relaxed">{aiAnalysis}</div>}
                      </div>
                   ) : activeStream === 'protocol' ? (
                     <div className="space-y-8 animate-fadeIn">
                        <div className="grid gap-4">
                          {[{ step: "01", title: "THE AUDIT" }, { step: "02", title: "THE ARCHITECTURE" }, { step: "03", title: "IMPLEMENTATION" }, { step: "04", title: "FREEDOM" }].map((p, idx) => (
                     ...
                     <div className="space-y-8 animate-fadeIn">
                        <div className="grid gap-4">
                          {deadlines.map((d, i) => (
                            <div key={i} className="flex justify-between items-center p-6 bg-black/60 border-2 rounded-[2rem] border-white/10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-10 animate-fadeIn pt-10">
                      ...
                      <button type="submit" className="w-full bg-brand-gold text-brand-900 rounded-[2.5rem] py-10 font-black uppercase tracking-[0.5em] text-base transition-all hover:scale-105 shadow-2xl mt-12">CONFIRM UPLINK</button>
                    </form>
                   )}
                </div>
              </div>
