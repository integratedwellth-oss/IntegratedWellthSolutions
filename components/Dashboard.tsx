...
const Dashboard: React.FC = () => {
  ...
  if (!user) return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 font-sans">
        <div className="max-w-md w-full bg-white/5 border border-white/10 p-12 rounded-[2.5rem] text-center">
    ...
    <div className="min-h-screen bg-slate-950 text-white font-sans pt-12 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-12 border-b border-white/10 pb-8">
          <div><h1 className="text-4xl font-black uppercase tracking-tighter">Intelligence Hub</h1><p className="text-brand-gold text-xs uppercase mt-2 font-bold tracking-widest">{user.email}</p></div>
              ...
              <tbody className="divide-y divide-white/5">
                {(activeTab === 'warroom' ? warRoomLeads : assessments).map((item) => (
                  <tr key={item.id} className="hover:bg-white/5 transition-colors">
                    <td className="px-8 py-5 text-white/40 font-mono text-xs">{item.timestamp?.toDate ? item.timestamp.toDate().toLocaleDateString() : 'N/A'}</td>
                    ...
                    {selectedLead.intelligence_report && Array.isArray(selectedLead.intelligence_report) ? (
                        ...
                        selectedLead.detailed_intelligence ? (
                           <div className="space-y-4">
                              <div className="p-4 bg-white rounded-2xl shadow-sm border border-brand-900/5"><p className="text-[10px] font-black text-brand-900/30 uppercase mb-1">Threat</p><p className="font-bold text-rose-600">{selectedLead.detailed_intelligence.immediate_threat}</p></div>
                              <div className="p-4 bg-brand-900 rounded-2xl shadow-sm"><p className="text-[10px] font-black text-white/30 uppercase mb-1">Solution</p><p className="font-black text-brand-gold uppercase">{selectedLead.detailed_intelligence.required_solution}</p></div>
                           </div>
                        ) : <p className="text-xs italic text-brand-900/40 text-center">No detailed data available for this legacy entry.</p>
                    )}
                  </div>
               </div>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <a href={`mailto:${selectedLead.email}`} className="flex items-center justify-center gap-3 bg-brand-900 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-brand-gold transition-all"><Mail size={16}/> Email Lead</a>
                  <a href={`https://wa.me/${selectedLead.whatsapp?.replace(/[^0-9]/g, '')}`} target="_blank" className="flex items-center justify-center gap-3 bg-[#25D366] text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest hover:opacity-90 transition-all"><MessageSquare size={16}/> WhatsApp</a>
               </div>
            </div>
          </div>
        </div>
