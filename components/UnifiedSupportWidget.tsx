...
import { X, Send, Loader2, Sparkles, MessageCircle, Zap, ShieldQuestion, ArrowRight, RefreshCcw } from 'lucide-react';
...
const UnifiedSupportWidget: React.FC = () => {
  ...
  const handleSend = async (textOverride?: string) => {
    ...
    try {
      ...
      for await (const chunk of responseStream) {
         // Correctly access the .text property as per GenerateContentResponse definition
      ...
      setMessages(prev => [...prev, { 
        role: 'model', 
        text: "### PROTOCOL INTERRUPTED\n\nI am having trouble maintaining the connection to the Sovereignty Core. This could be due to restricted network parameters or an expired security token.\n\n**Recovery Options:**\n* **Retry Connection** below\n* Contact **Marcia via WhatsApp** for immediate tactical support.", 
        timestamp: Date.now() 
      }]);
    } finally {
      setIsLoading(false);
  ...
  const renderMarkdown = (text: string) => {
    ...
    return lines.map((line, i) => {
      ...
      return line.trim() ? <p key={i} className="mb-3 text-sm leading-relaxed text-brand-900/90">{renderBold(line)}</p> : <div key={i} className="h-2" />;
    });
  };
  ...
  const renderBold = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="font-extrabold text-brand-900 underline decoration-brand-gold/20">{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
  ...
  return (
    <div className="fixed bottom-24 right-6 md:bottom-8 md:right-8 z-[500] font-sans">
              ...
              <div className="flex flex-col h-full max-h-[550px]">
                ...
                <div className="p-4 bg-white border-t border-brand-900/5 flex gap-3">
                  ...
                  <input 
                    ...
                    placeholder="Ask about 2026 compliance..." 
                    className="flex-1 bg-brand-50 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-900/10" 
                  />
                  <button 
                    onClick={() => handleSend()} 
                    disabled={isLoading || !input.trim()}
                    className="bg-brand-900 text-white p-3.5 rounded-2xl hover:scale-105 transition-all disabled:opacity-30 disabled:hover:scale-100"
                  >
                    <Send size={20} />
                  </button>
                </div>
      ...
      <button 
        ...
        className={`w-16 h-16 md:w-20 md:h-20 rounded-[2rem] flex items-center justify-center shadow-2xl transition-all duration-700 hover:rotate-[360deg] ${isOpen ? 'bg-brand-900 text-white' : 'bg-white text-brand-900 hover:scale-110'}`}
      >
