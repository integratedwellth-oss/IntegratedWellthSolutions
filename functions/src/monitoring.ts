...
const sendMetaMessage = async (toNumber: string, text: string): Promise<void> => {
  ...
  await fetch(`https://graph.facebook.com/v21.0/${PHONE_NUMBER_ID}/messages`, {
    ...
    body: JSON.stringify({ messaging_product: "whatsapp", recipient_type: "individual", to: toNumber, type: "text", text: { body: text } })
  });
};
...
export const weeklySecurityAudit = onSchedule({
  ...
  // 1. AI Audit Logs - count requests in last 7 days
  try {
    ...
    if (auditSnapshot.size > 500) {
      report += `⚠️ HIGH: Unusual spike detected!\n`;
    }
  } catch (e) {
    report += `🤖 *AI Chat Requests:* Error fetching\n`;
  }

  // 2. Rate Limits - anyone capped at 30/hour
  try {
    ...
    if (rateSnapshot.empty) {
      ...
      report += `🚦 *Rate Limit Hits:* ${rateSnapshot.size} user(s) capped\n`;
      rateSnapshot.docs.slice(0, 3).forEach(doc => {
        report += `   - ${doc.id}: ${doc.data().count} requests\n`;
      });
    }
  } catch (e) {
    report += `🚦 *Rate Limit Hits:* Error fetching\n`;
  }

  // 3. Dashboard Access - check user_logs for non-admin access
  try {
    const logSnapshot = await db.collection("user_logs")
      .where("timestamp", ">=", admin.firestore.Timestamp.fromDate(oneWeekAgo))
      .where("action", "==", "DASHBOARD_ACCESS")
      .get();
    const adminEmails = ["enquiries@integratedwellth.co.za", "marcia@integratedwellth.co.za"];
    let unauthorized = 0;
    ...
    if (unauthorized === 0) {
      ...
      report += `🚨 *Dashboard Access:* ${unauthorized} unauthorized attempt(s)!\n`;
    }
  } catch (e) {
    report += `🔑 *Dashboard Access:* Error fetching\n`;
  }
  ...
  if (adminPhone) {
    ...
    console.log("Security report sent to admin.", report);
  } else {
    console.log("ADMIN_PHONE_NUMBER not set. Report:", report);
  }
});
